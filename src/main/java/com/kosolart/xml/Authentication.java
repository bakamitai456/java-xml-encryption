...
public class Authentication {
  ...
  public static void main(String[] args) {
    ...
    String oAuthPayload = null; // pass Authentication payload

    try {

      String JKS_FILE_PATH = null;// Initialize with JKS file path -- for
// eg.,“C:\API\Cert\ClientKeyStore.jks”
      String KEYSTORE_PWD = null; // Initialize with Key store password;
// -- For eg., “pass123”
      String CLIENT_SIGN_ALIAS = null; // Initialize with Client Sign
// certificate key alias
      String CITI_ENCRYPT_ALIAS = null;// Initialize with CITI"s
// encryption certificate key
// alias
      String signedEncryptedoAuthRequest = encrypt(oAuthPayload, JKS_FILE_PATH, KEYSTORE_PWD,
      ...
      String endUrl = null; // For
// eg.,"https://tts.sandbox.apib2b.citi.com/citiconnect/sb/authenticationservices/v1/oauth/token";
      String clientId = null; // For
// eg.,"9a9069d1-ed93-4d40-8b60-1e56a53899df"
      String secretkey = null; // - For eg.
// “F5yR7jdsfrethzserQ8iN6tU7xQ5sX8rQ8ofghqwnP3fdslY0rJ8tQ8vO6hhI7eE4rA1nSui73d6”
      String responseISOPayXMLPayload = callAuthenticationAPI(signedEncryptedoAuthRequest,
          SSL_CERT_PATH, SSL_PWD,
          PROXY_URL, port, endUrl, clientId, secretkey);

      String CLIENT_DECRYPT_ALIAS = null; // Initialize with Client
// decryption certificate key
// alias
      String CITI_VERF_ALIAS = null; // Initialize with CITI"s signing
// certificate key alias
      Document oAuthDoc = decrypt(responseISOPayXMLPayload, JKS_FILE_PATH, KEYSTORE_PWD,
      ...
      transformer.transform(new DOMSource(oAuthDoc), new StreamResult(writer));
      String verifiedDecryptedAuthResponse = StringEscapeUtils
          .unescapeXml(writer.getBuffer().toString());
      System.out.println("oAuth Decrypted Response\n" + verifiedDecryptedAuthResponse);

      String oAuth = parseResponse(oAuthDoc, "", "//access_token/text()");
...
// Convert the document object to String value
  ...
  private static String callAuthenticationAPI(String signedEncryptedoAuthRequest,
      String sslCertFilePath,
      String certPwd, final String proxyURL, final int port, String oAuthURL, String clientID,
      String secretkey) {
    String response = "";
    System.out.println("<<<<< &&&&& in callPaymentInitiationAPI &&&&& >>>>>> " + oAuthURL);
    try {
...
//          + Base64.encodeBase64String((clientID + ":" + secretkey).getBytes())
//          .replaceAll("(\r|\n)", ""));
//
//      ClientResponse clientResponse = builder
//          .post(ClientResponse.class, signedEncryptedoAuthRequest);
...
// Convert the document object to String value
  ...
  private static String parseResponse(Document responeDoc, String type, String tagName)
    ...
    if (docRoot == null || docRoot.getNodeName() == null) {
      errorInResponse = "Response Message Doesnt have expected Information";
    } else {
     ...
     * logger.info("Output Auth Response String ==> " , parseError); else
     */
